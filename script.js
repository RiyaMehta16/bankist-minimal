...
const btnsOpenModal = document.querySelectorAll('.btn--show-modal'); //gives a node-list
...
window.addEventListener('scroll', function (e) {
  //scrollong event shouldnt be used as it causes performance issues
  if (window.scrollY > initialCoords.top) nav.classList.add('sticky');
  else nav.classList.remove('sticky');
});
//STICKY NAVIGATION: INTERSECTION OBSERVER API
const obsCallback = function (entries, observer) {
//enteries means threshold values
//observer means the object itself i.e. obsOptions here
...
*/
  ...
  const activateDot = function (slide) {
    //removing from all dots
    document
      .querySelectorAll('.dots__dot')
      .forEach(dot => dot.classList.remove('dots__dot--active'));
    //adding to one we want
    document
...
message.innerHTML =
  'We use cookies for improved functionality. <button class="btn btn--close-btn">Got it!<button>';
header.append(message);

// header.prepend(message.cloneNode(true));
//deleting element
document
  .querySelector('.btn--close-btn')
...
// h1.parentNode.style.backgroundColor = '#666';
